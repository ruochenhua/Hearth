...
const page = `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>围炉 Hearth 控制面板</title><style>body{margin:0;background:#f7f2ea;color:#3f2a1a;font:16px system-ui,-apple-system,Segoe UI,sans-serif}.card{max-width:680px;margin:8vh auto;padding:38px;background:white;border:1px solid #e6dccd;border-radius:18px;box-shadow:0 16px 50px #3f2a1a12}h1{margin:0 0 8px;font-size:32px}p{color:#6f858e}.status{padding:14px 16px;border-radius:10px;background:#f1ead9;margin:26px 0}.dot{display:inline-block;width:10px;height:10px;border-radius:50%;background:#c17c5e;margin-right:8px}.up .dot{background:#529172}.buttons{display:flex;flex-wrap:wrap;gap:10px}button{border:0;border-radius:9px;padding:12px 17px;background:#3f2a1a;color:white;font-size:15px;cursor:pointer}button.secondary{background:#e9dfcb;color:#3f2a1a}button:disabled{opacity:.5}small{color:#94826e;display:block;margin-top:25px;line-height:1.7}.url{display:block;margin:6px 0;color:#a06a2f}</style></head><body><main class="card"><h1>围炉</h1><p>Hearth · 家庭相册控制面板</p><div id="status" class="status"><span class="dot"></span><b>Checking…</b></div><div id="urls"></div><div class="buttons"><button id="start">Start album</button><button id="stop" class="secondary">Stop album</button><button id="repair" class="secondary">Repair LAN access</button><button id="open" class="secondary">Open album</button></div><small>此控制面板只运行在本机。 It detects Windows, macOS, or Linux automatically and keeps firewall details inside the platform adapter.</small></main><script>const $=s=>document.querySelector(s);async function api(path,method='GET'){const r=await fetch(path,{method,headers:{'Content-Type':'application/json'}});const b=await r.json();if(!r.ok)throw Error(b.error||'Action failed');return b}async function refresh(){try{const s=await api('/api/status');$('#status').className='status '+(s.running?'up':'');$('#status').innerHTML='<span class="dot"></span><b>'+(s.running?'Running':'Stopped')+'</b> · '+s.platform;$('#urls').innerHTML='<a class="url" href="http://localhost:3080">Local: http://localhost:3080</a>'+s.addresses.map(a=>'<span class="url">LAN: http://'+a+':3080</span>').join('');$('#start').disabled=s.running;$('#stop').disabled=!s.running}catch(e){$('#status').innerHTML='<span class="dot"></span><b>Control agent unavailable</b>'}}async function action(path){try{for(const b of document.querySelectorAll('button'))b.disabled=true;await api(path,'POST');await refresh()}catch(e){alert(e.message)}finally{await refresh()}}$('#start').onclick=()=>action('/api/start');$('#stop').onclick=()=>action('/api/stop');$('#repair').onclick=()=>action('/api/repair');$('#open').onclick=()=>window.open('http://localhost:3080','_blank');refresh();setInterval(refresh,2500)</script></body></html>`;
